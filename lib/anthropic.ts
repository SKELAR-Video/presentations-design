...
- rows_dot → 2 або 3 тези одна під одною, кожна з червоною крапкою зліва. Слоти: ПУНКТ_1, ПУНКТ_2, ПУНКТ_3? (опціонально — при двох пунктах лиши null). Кожен ПУНКТ — цілий рядок на всю ширину слайда, тому сюди йдуть ФРАЗИ І РЕЧЕННЯ, які у вузькі колонки не влізли б. Обирай замість two_columns/three_columns, коли пункти читаються послідовно (один за одним), а не як паралельні колонки. Тільки dark-тема.
...
- "(підзаголовок)" — фрагмент стоїть на всю ширину між заголовком і колонками. Це ПІДЗАГОЛОВОК слайда: признач його у слот ПІДЗАГОЛОВОК, НІКОЛИ у ПІДПИС_N (мітка колонки) і НІКОЛИ у КОЛОНКА_N/КАРТКА_N.
...
- "(колонка N)" — фрагмент належить до N-ї паралельної групи. Це або окрема фігура, що візуально стоїть поруч з іншими такими ж фігурами, або блок тексту, відокремлений порожнім рядком всередині ОДНІЄЇ фігури (напр. "Залучення талантів\n...\n\nРепутація\n..." — дві групи в одному текстовому боксі). В обох випадках фрагменти з різним N — ПАРАЛЕЛЬНІ, окремі групи (типовий кандидат на two_columns/three_columns з КОЛОНКА_N по кожній — цілий фрагмент іде в один КОЛОНКА_N, включно з першим рядком-назвою), а не один суцільний текст у ТЕКСТ/title_body.

Правила:
1. Кількість слайдів у виході = кількість у вході. Без злиття, без розбиття.
2. Перший → cover (якщо є підзаголовок або дата) або cover_title_only (якщо є лише заголовок). Останній → closing. closing: ЗАГОЛОВОК = індекс короткого заголовного рядка; якщо на цьому слайді є ЩЕ фрагменти (підсумковий текст, тези, список) — ОБОВ'ЯЗКОВО признач їх усі в ПІДЗАГОЛОВОК (масивом індексів через кому, дослівно, нічого не пропускай) — НІКОЛИ не залишай зайві фрагменти без слоту.
...
12. rows_dot → 2 або 3 тези одна під одною, кожна з червоною крапкою зліва. ПУНКТ_1, ПУНКТ_2 обов'язкові, ПУНКТ_3 — тільки якщо пунктів три (інакше null). По ОДНОМУ індексу на слот. Рядок іде на всю ширину слайда, тож підходить для фраз і речень; обирай замість two_columns/three_columns, коли пункти читаються послідовно, а не як паралельні колонки.
13. title_photo → Як title_body з фото справа. ЗАГОЛОВОК(обов'язково) + ТЕКСТ?(опціонально) + ФОТО?(URL якщо є в тексті, інакше null). Обирай замість title_body коли тема виграє від візуального акценту.

## markers — що виділяти білим у колонках
Для КОЖНОЇ колонки (КОЛОНКА_N / КАРТКА_N), у якій ≥2 рядків, додай відповідь у "markers":
  true  — перший рядок називає ХТО або ЩО це за група ("Викладачі/Голови студпарламентів", "Студенти", "«Зіркові» учні шкіл"), а рядки під ним описують цю групу
  false — перший рядок називає ДІЮ або є звичайним пунктом, рівним іншим ("Підтримка проявів бренду" серед інших дій: відбір, організація, розповсюдження)
Ключове питання: це НАЗВА ГРУПИ чи ОДНА З ДІЙ? Дія — завжди false, навіть якщо коротка.
Це рішення про ЗМІСТ, не про довжину: короткий перший рядок сам по собі ще не робить його назвою категорії.
  ...
  const slidesText = slides
    .map((s, i) => {
      const items = s.texts.map((t, ti) => {
        ...
        const subTag = s.subtitles?.[ti] ? ' (підзаголовок)' : ''
        return `  [${ti}]${colTag}${subTag} ${JSON.stringify(t.slice(0, 300))}`
      })
