...
- two_columns → СТРОГО 2 паралельні тези (не більше, не менше).
...
- columns_flex → ЄДИНИЙ варіант коли пунктів рівно 4 і жоден не є метрикою. Також підходить для 2–3 пунктів якщо потрібна гнучкість. Слоти: КОЛОНКА_1, КОЛОНКА_2, КОЛОНКА_3? (опціонально), КОЛОНКА_4? (опціонально). НІКОЛИ не використовуй КАРТКА_N для columns_flex.
...
- title_photo → Слайд із великим заголовком зліва та фото справа (половина екрану). ЗАГОЛОВОК(≤80) — головна теза. ТЕКСТ?(≤300) — опціональний розвиток думки. ФОТО?(≤300) — URL фото (http...) якщо явно вказано в ТЗ, інакше null. Обирай для сильних відкриттів теми, ключових висновків, closing із візуалом.
...
- **КОЛОНКА_N і КАРТКА_N — РІВНО ОДИН фрагмент кожен.** Ніколи не присвоюй масив [i, j, ...] у КОЛОНКА_N або КАРТКА_N. Якщо пунктів 4 і вони не є метриками — ОБОВ'ЯЗКОВО columns_flex (КОЛОНКА_1..4), НІКОЛИ не three_columns або three_columns_num.
...
10. КОЛОНКА_N і КАРТКА_N — РІВНО ОДИН індекс кожен (ніколи масив). two_columns → СТРОГО 2. three_columns / three_columns_num → СТРОГО 3. Якщо пунктів рівно 4 і не метрики — ОБОВ'ЯЗКОВО columns_flex (КОЛОНКА_1..4), НІКОЛИ three_columns. bento_right_N → КАРТКА_N (не КОЛОНКА_N): bento_right_2→КАРТКА_1/2, bento_right_3→КАРТКА_1/2/3, bento_right_2x2→КАРТКА_1/2/3/4.
...
12. title_photo → ЗАГОЛОВОК(обов'язково) + ТЕКСТ?(опціонально) + ФОТО?(URL якщо є в тексті, інакше null). Обирай для слайдів з однією сильною тезою і потенційно емоційним візуалом.
