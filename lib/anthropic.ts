...
- columns_flex → 2, 3 або 4 паралельних пункти / кроки (не метрики). Слоти: КОЛОНКА_1, КОЛОНКА_2, КОЛОНКА_3? (опціонально), КОЛОНКА_4? (опціонально). НІКОЛИ не використовуй КАРТКА_N для columns_flex.
...
- **КОЛОНКА_N і КАРТКА_N — РІВНО ОДИН фрагмент кожен.** Ніколи не присвоюй масив [i, j, ...] у КОЛОНКА_N або КАРТКА_N. Для 2–4 паралельних пунктів (не метрики) — ЗАВЖДИ columns_flex (КОЛОНКА_1..N).
...
10. КОЛОНКА_N і КАРТКА_N — РІВНО ОДИН індекс кожен (ніколи масив). two_columns → СТРОГО 2. Для 2–4 паралельних пунктів (не метрики) — ЗАВЖДИ columns_flex (КОЛОНКА_1..N). bento_right_N → КАРТКА_N (не КОЛОНКА_N): bento_right_2→КАРТКА_1/2, bento_right_3→КАРТКА_1/2/3, bento_right_2x2→КАРТКА_1/2/3/4.
...
export async function mapSlides1to1(
  ...
  const RETIRED_COMPOSITIONS = new Set(['three_columns', 'three_columns_num'])
  // Compact catalog — slot names only, no max_chars (LLM assigns indices, not text)
  const catalogDescription = PHASE0_COMPOSITIONS.filter(c => !RETIRED_COMPOSITIONS.has(c.id)).map((c) => {
