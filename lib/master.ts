...
function buildLayout(compId: string, slideId: string, bgColor: RGB, idx: number): object[] {
  ...
  switch (compId) {
    ...
    case 'cover': {
      // H1_MAX / SUB_MAX must match compositions.ts cover slot max_h values.
      // All three positions are overridden dynamically in lib/google.ts buildCoverFloatRequests.
      const H1_MAX  = 400
      const SUB_MAX = 200
      push(
...
// ─── Reusable builder ───────────────────────────────────────────────────────
// The template is not a precious artifact — it is the output of the code above, built
// from PHASE0_COMPOSITIONS every time. That matters for permissions: a deck this app
// creates is covered by the narrow drive.file scope, while reading somebody else's
// shared template is what forces the broad drive.readonly on every user.
//
// Marked with appProperties so it can be found again without a name search, exactly as
// the output folder is (see ensureDeckFolder): a name can collide with the user's own
...
export async function createMasterDeck(
  ...
  console.log(`[master] built own template ${presentationId}`)
